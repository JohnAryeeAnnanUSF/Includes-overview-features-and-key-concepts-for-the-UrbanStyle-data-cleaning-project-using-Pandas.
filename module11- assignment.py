...
def plot_category_performance_by_location():
    ...
    category_location_sales = latest_data.groupby(['Location', 'Category'])['Sales'].sum().unstack()
...
def plot_sales_composition_by_location():
    ...
    location_category_percent = location_category_sales.div(location_category_sales.sum(axis=1), axis=0) * 100

    fig, ax = plt.subplots(figsize=(10, 6))
    location_category_percent.plot(kind='bar', stacked=True, ax=ax)
...
def plot_ad_spend_vs_sales():
    ...
    ax.plot(x, best_fit, linestyle='--', linewidth=2, label='Best-Fit Line')

    # Annotate one notable outlier
    sales_df_copy = sales_df.copy()
    sales_df_copy['Predicted'] = slope * sales_df_copy['AdSpend'] + intercept
    sales_df_copy['Residual'] = abs(sales_df_copy['Sales'] - sales_df_copy['Predicted'])
    outlier_idx = sales_df_copy['Residual'].idxmax()
    outlier = sales_df_copy.loc[outlier_idx]

    ax.annotate(
        f"{outlier['Location']} - {outlier['Category']}",
...
def plot_ad_efficiency_over_time():
    ...
    max_idx = efficiency.idxmax()
    max_val = efficiency.max()
    ax.annotate(
        f"Highest: {max_val:.2f}",
        xy=(list(efficiency.index).index(max_idx), max_val),
        xytext=(10, 10),
        textcoords='offset points'
    )
...
def plot_purchase_by_age_group():
    ...
    data_for_boxplot = [
        customer_df_copy[customer_df_copy['AgeGroup'] == label]['PurchaseAmount']
        for label in labels
    ]
...
def plot_sales_by_price_tier():
    ...
    explode = [0.08 if value == tier_sales.max() else 0 for value in tier_sales]
...
def plot_category_market_share():
    ...
    explode = [0.08 if value == category_sales.max() else 0 for value in category_sales]
...
def main():
    ...
    print("1. Overall sales trend upward over time, with the strongest sales appearing in Q4.")
    print("2. Miami appears to be the strongest-performing location in total sales.")
    print("3. Electronics hold the largest share of sales across the company.")
    print("4. Higher advertising spend generally aligns with higher sales, showing a positive relationship.")
    print("5. Mid-range purchases make up the largest share of customer spending.")
    print("6. Customer age patterns vary by location, which could help target marketing strategies.")
    print("\nBUSINESS RECOMMENDATIONS:")
    print("- Increase focus on high-performing categories like Electronics.")
    print("- Study Miami's performance to see what can be copied in other locations.")
    print("- Continue tracking advertising efficiency to improve return on marketing spend.")
    print("- Use customer age differences by location to tailor promotions and product placement.")
